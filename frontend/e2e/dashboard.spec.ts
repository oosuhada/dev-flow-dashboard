...
const snapshot = (repo: string) => ({
  ...
  pulls: repo === "Biz-CollabCraft/gen_data" ? [] : [
  ...
  commits: repo === "Biz-CollabCraft/gen_data" ? [] : [{ sha: "a".repeat(40), message: "base commit", author: "Dev", timestamp: "2026-08-18T01:00:00Z", parents: [], branch: "stack-a", prNumber: 21 }],
});
...
test("root renders PR graph, dependency, drawer, commit graph and repository selector", async ({ page }) => {
  ...
  await expect(page.getByText("Open PRs").locator("..").getByText("0")).toBeVisible();
});
